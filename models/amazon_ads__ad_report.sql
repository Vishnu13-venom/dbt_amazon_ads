...
with
    ...
    sb_campaigns as (select * from {{ var("sb_campaign_history") }}),

    sponsored_brand_report as (
        select
            'sponsored_brands' as ad_type,
            date_day,
            cast(sc.portfolio_id as string) as portfolio_id,
            sc.name as portfolio_name,  -- ✅ Add this line
            null as campaign_name,
            cast(sb.campaign_id as string) as campaign_id,
            sb.ad_group_id,
            sb.ad_id,
            null as ad_group_name,
        ...
        left join sb_campaigns sc on sb.campaign_id = sc.id
    ),

    report as (
        select *
        from sponsored_product_report
        union all
        select *
        from sponsored_brand_report
    ),

    account_info as (
        select * from {{ var("profile") }} where _fivetran_deleted = false
    ),

    portfolios as (select * from {{ source("amazon_ads", "portfolio_history") }}),

    campaigns as (select * from {{ var("campaign_history") }}),

    ad_groups as (select * from {{ var("ad_group_history") }}),

    ads as (select * from {{ var("product_ad_history") }}),
    ...
    final as (
        ...
        left join ads pa on pa.ad_id = report.ad_id

        left join ad_groups ag on ag.ad_group_id = report.ad_group_id

        left join campaigns ca on ca.campaign_id = report.campaign_id
        ...
        left join
            account_info ai on ai.profile_id = report.profile_id
